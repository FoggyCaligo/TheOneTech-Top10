...
7. 각 세부 군집의 중심점에서 먼 기사는 이질 기사로 보고 분리합니다. 분리된 기사 수가 충분하면 한 번 더 재군집화합니다.
8. 세부 군집들의 임베딩 중심점이 가까우면 다시 상위 이슈로 병합합니다.
9. 상위 이슈별 TF-IDF 키워드, 대표 기사, 점수, 지도 좌표를 생성합니다.

중복 판정은 키워드가 아니라 원문을 봅니다. 현재는 `body` 완전중복 제거와 `dedupe_text(title + body)` 임베딩 유사도 제거를 함께 사용합니다. 그래서 제목만 달라도 본문이 완전히 같으면 먼저 제거됩니다. 앱의 분석 버튼 위 미리보기 표도 본문 완전중복을 제외한 상태로 보여줍니다.
...
분석은 토픽 단위가 아니라 기사 단위로 시작합니다. 중복 제거는 원문 기반으로 하고, 군집화는 정규화된 키워드 텍스트 기반으로 합니다. 이후 세부 군집 내부 outlier를 분리하고, outlier가 충분하면 다시 재군집화합니다. 마지막으로 세부 군집의 임베딩 중심점끼리 비교해 가까운 군집들을 상위 이슈로 병합합니다. 토픽명은 상위 이슈가 만들어진 뒤 TF-IDF 키워드로 붙이는 후처리 라벨입니다.

앱의 `상위 이슈 병합 유사도`는 이 2단계 병합 기준입니다. 낮출수록 `비트코인 가격`, `가상화폐 규제`, `거래소`처럼 분리된 세부 군집이 하나의 넓은 이슈로 합쳐질 가능성이 커집니다. 높일수록 세부 군집을 더 분리해서 봅니다.

앱의 `군집 내부 이질 기사 제거 기준`은 세부 군집 안에서 중심점과 먼 기사를 분리하는 기준입니다. 높일수록 군집 내부 순도가 올라갈 수 있지만, 같은 이슈 안의 다양한 기사도 더 쉽게 떨어져 나갈 수 있습니다.
