...
BIGKINDS_DOWNLOAD_PATH=Top10_experiment/NewsResult_20260116-20260716_6month.xlsx
...
7. 각 군집의 중심점에서 먼 기사는 이질 기사로 보고 분리합니다. 분리된 기사 수가 충분하면 한 번 더 재군집화합니다.
8. 군집별 라벨, 대표 기사, 점수, 지도 좌표를 생성합니다. 라벨은 기사 한 편 안의 반복 횟수가 아니라, 군집 안 여러 기사에 등장한 문서 빈도와 전체 기사 대비 희소성을 함께 봅니다.
...
- 추가: 같은 키워드 서명이 과도하게 반복되는 템플릿성 입력은 군집화 전에 일부만 남깁니다.
...
분석은 토픽 단위가 아니라 기사 단위로 시작합니다. 중복 제거는 원문 기반으로 하고, 군집화는 정규화된 키워드 텍스트 기반으로 합니다. 이후 군집 내부 outlier를 분리하고, outlier가 충분하면 다시 재군집화합니다. 토픽명은 군집이 만들어진 뒤 붙이는 후처리 라벨이며, 군집 안 문서 빈도와 전체 기사 대비 희소성을 함께 반영합니다.

앱의 `군집 내부 이질 기사 제거 기준`은 군집 안에서 중심점과 먼 기사를 분리하는 기준입니다. 높일수록 군집 내부 순도가 올라갈 수 있지만, 같은 이슈 안의 다양한 기사도 더 쉽게 떨어져 나갈 수 있습니다.
