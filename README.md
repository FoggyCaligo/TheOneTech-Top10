...
8. 군집별 TF-IDF 키워드, 대표 기사, 점수, 지도 좌표를 생성합니다.
...
분석은 토픽 단위가 아니라 기사 단위로 시작합니다. 중복 제거는 원문 기반으로 하고, 군집화는 정규화된 키워드 텍스트 기반으로 합니다. 이후 군집 내부 outlier를 분리하고, outlier가 충분하면 다시 재군집화합니다. 토픽명은 군집이 만들어진 뒤 TF-IDF 키워드로 붙이는 후처리 라벨입니다.
